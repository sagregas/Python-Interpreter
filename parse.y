...
%{
  ...
  #include "tableManager.h"
	...
	extern char *yytext;
        std::string name;
        std::vector<Ast*> stmtVec;
        std::vector<Ast*>* funcStmt;
...
%token AMPEREQUAL AMPERSAND AND AS ASSERT AT BAR BREAK
...
%token IF IMPORT IN INDENT IS LEFTSHIFT LEFTSHIFTEQUAL LESS
%token LESSEQUAL LPAR MINUS MINEQUAL NEWLINE NOTEQUAL
...
%token TRY VBAREQUAL WHILE WITH
%token <ast> LSQB LBRACE BACKQUOTE STRING LAMBDA YIELD NOT NUMBER NAME
%token <op> TILDE  
%type <op> pick_PLUS_MINUS pick_multop pick_unop augassign star_trailer
%type <ast> print_stmt arith_expr term factor power atom opt_test opt_yield_test star_EQUAL pick_yield_expr_testlist yield_expr testlist test opt_test_2 PRINT plus_COMMA_test or_test opt_IF_ELSE lambdef  plus_STRING pick_yield_expr_testlist_comp IF and_test testlist_comp COMMA not_test comparison expr xor_expr and_expr shift_expr plus_stmt stmt expr_stmt EQUAL simple_stmt compound_stmt small_stmt if_stmt while_stmt for_stmt try_stmt with_stmt funcdef classdef decorated decorators del_stmt pass_stmt flow_stmt import_stmt global_stmt exec_stmt assert_stmt decorator break_stmt continue_stmt return_stmt raise_stmt yield_stmt import_name import_from AT DEL PASS BREAK CONTINUE RETURN RAISE IMPORT FROM GLOBAL EXEC ASSERT WHILE FOR TRY WITH CLASS
...
funcdef // Used in: decorated, compound_stmt
	: DEF NAME parameters COLON suite     {
                                                 TableManager::getInstance().decrScope();
                                                 Ast* f = new FunctionNode('1',($2)->getKey(),funcStmt);
                                                 f->setScope(TableManager::getInstance().getCurrScope());
                                                 TableManager::getInstance().insertSymbol(($2)->getKey(),TableManager::getInstance().getCurrScope(),f); 
                                              }
	;
...
expr_stmt // Used in: small_stmt
	: testlist augassign pick_yield_expr_testlist
          { 
            Ast* temp;
            std::string val = ($$)->getKey();
            if($2 == 'A'){
                temp = new PlusNode('+',new IdNode('I', 't',val), $3);
                $$ = new AssNode('V',new IdNode('I','t',val), temp);
             }
             if($2 == 'S'){
                temp = new MinusNode('-',new IdNode('I','t',val), $3);
                $$ = new AssNode('V',new IdNode('I','t',val), temp);
             }
             if($2 == 'M'){
                temp = new MultNode('*',new IdNode('I','t',val), $3);
                $$ = new AssNode('V',new IdNode('I','t',val), temp);
             }
             if($2 == 'D'){
                temp = new DivNode('/',new IdNode('I','t',val), $3);
                $$ = new AssNode('V',new IdNode('I','t',val), temp);
             }
             if($2 == 'P'){
                temp = new ModNode('%',new IdNode('I','t',val), $3);
                $$ = new AssNode('V',new IdNode('I','t',val), temp);
             }
             if($2 == 'X'){
               temp = new IntDivNode('!',new IdNode('I','t',val), $3);
                $$ = new AssNode('V',new IdNode('I','t',val), temp);
             }
          }
...
star_EQUAL // Used in: expr_stmt, star_EQUAL
	: EQUAL pick_yield_expr_testlist star_EQUAL {
                                                       name = ($$)->getKey();
                                                       $$ = new AssNode('V',new IdNode('I','t', name), $2);
                                                    }
	| %empty{$$ = nullptr;}
	;
...
suite // Used in: funcdef, if_stmt, star_ELIF, while_stmt, for_stmt, 
	...
	: simple_stmt 
	| NEWLINE INDENT {TableManager::getInstance().incrScope();} plus_stmt DEDENT
	;
plus_stmt // Used in: suite, plus_stmt
	: stmt plus_stmt  { if($1 != NULL)
...
power // Used in: factor
	: atom star_trailer DOUBLESTAR factor  {$$ = new ExpNode('^',$1, $4);}
	| atom star_trailer
          {
            if($2 == 'c') {
